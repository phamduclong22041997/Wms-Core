...
class WatcherCollection {
    ...
    async publish() {
        try {
            let redisClient = null;//Redis.getClient();
            if (redisClient) {
                let route = await Redis.getItem(`QUEUE_${process.env.APP_NAME}${this.QUEUE_SUFFIX}`);
                if (!route) {
                    Redis.setItem(`QUEUE_${process.env.APP_NAME}${this.QUEUE_SUFFIX}`, process.env.APP_INSTANCE, 432000);
                }
                await sleep(Math.floor(Math.random() * 100) + 1);
                route = await Redis.getItem(`QUEUE_${process.env.APP_NAME}${this.QUEUE_SUFFIX}`);
                if (!route) {
                    this.publish();
                    return;
                }
                if (redisClient && route == process.env.APP_INSTANCE) {
                    this.processHandle();
                }
            } else {
                this.processHandle();
        ...
        if (this.enableValidate) {
            try {
                await DB.getCollection("ValidateLogs").create({
                    key: `${this.coll}${doc._id.toString()}`,
                    db: DB._db['name']
                });
            }catch(err){
                return;
    ...
    connect() {
        setTimeout(async () => {
            await Redis.removeItem(`QUEUE_${process.env.APP_NAME}${this.QUEUE_SUFFIX}`);
            this.supervisor();
        }, 1000);
